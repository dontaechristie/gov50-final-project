...
elite_college <- df |>
   mutate(
    elite_college = if_else(V12 == "Ivy Plus", "Extremely Selective", "Not Extremely Selective")
  ) 
tibble_output <- df |>

  group_by(elite_college) |>
  summarise(mean_kid_median = mean(kid_median, na.rm = TRUE),
            mean_parent_median = mean(parent_median, na.rm = TRUE)) |>
  pivot_wider(names_from = elite_college, values_from = c(mean_kid_median, mean_parent_median))
